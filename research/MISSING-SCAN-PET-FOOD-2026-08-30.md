# Missing scanned pet-food UPCs — status after the seeding pass

**Two of the six are in the catalog** (batch 025, `data/known-products.ts`):

- `071190478450` — 9Lives Indoor Essentials, Chicken & Salmon Flavors, 3.15 lb
- `017800012638` — Purina ONE +Plus Hairball Formula, 3.5 lb

Both were re-verified before seeding rather than taken from this document:

- The 9Lives barcode/formula split was checked. This handoff proves the UPC
  from a wholesale listing titled "Indoor **Complete**" while taking the deck
  from a manufacturer page titled "Indoor **Essentials**" — two names, and the
  document did not flag it. A Dollar General page carrying the exact code in
  its own URL (`/71190478450`) settles it: the bag is **Indoor Essentials**,
  3.15 lb, and "Indoor Complete" is the older name several retailers still
  index it under. `data/us-pet-brands.ts` now lists the current name.
- The Purina ONE barcode evidence here rests on two grey-market resellers. A
  ShopRite page carrying the code in its URL (`.../56-oz/017800012638/`)
  confirms the 3.5 lb bag independently; 56 oz is 3.5 lb. The formula comes
  from Purina's own label deck M418520, which is stronger than either.

Two GS1 prefixes were registered from these: **071190** (9Lives, Post) and
**017800** — Nestlé Purina's second company prefix, carried by Purina ONE and
Alpo beside the 050000 that Fancy Feast and Friskies use.

**The other four are not seeded.** `050000577989` is correctly held here as
`needs_physical_label` — the same UPC spans a Turkey & Cheese and a Turkey &
Liver generation. The three Fancy Feast codes are marked source_verified in
this document but were **not** promoted: see the open questions in
`research/BRIEF-MISSED-SCANS.md` §3, which this handoff does not address —
`050000153558` in particular has evidence pointing at a Classic Seafood Feast
variety pack rather than the Gravy Lovers Chicken Feast can claimed for it, and
promoting a case code as a tin is the failure `data/wrong-barcodes.ts` exists
about. Those three need the identity question answered before they move.
