# Weruva — status after the seeding pass

**All 133 `source_verified` records are in the catalog** (batch 027): 85
products under 133 barcodes. The ledger passed `scripts/check-ledger.mjs` after
the checker itself was corrected — see below — with 11 warnings, every one of
them a real question and every one answered.

Three things had to be decided before seeding, none of which this handoff
covers:

1. **Ten records are B.F.F., not Weruva.** The ledger files them as brand
   "Weruva" with `product_line` "B.F.F. OMG" or "B.F.F. Play".
   `data/us-pet-brands.ts` has held **B.F.F.** as its own brand — owner Weruva,
   ranges OMG and Play — since long before this campaign, and a shopper reads
   the tin that way. Left as written they would have filed under "Other" and
   the seeding test would have refused them. Re-branded on the way in.

2. **The whole Wx line is not a complete diet**, which is the most important
   fact in this batch and is buried in the verification notes rather than
   raised as a conflict. Weruva's own pages state intermittent or supplemental
   feeding only, and inadequate phosphorus for the AAFCO profiles at any life
   stage. Sold off a shelf, so `isVeterinaryDiet` correctly does not fire.
   Judged as an everyday complete diet it would be marked down for exactly the
   restriction it is bought for. `lib/nutrition-role.ts` gains
   `KNOWN_COMPLEMENTARY_LINES` for it; all nine Wx products now read
   `complementary`.

3. **Two Freeze Dried records are filed `food_form: "wet"`** beside an 8%
   moisture guarantee. The seed reads the form off the moisture, so both are
   stored as dry.

Three GS1 prefixes registered — 878408, 813778, 810028 — the most any single
maker here has, and none of them retired. Fifteen ranges into the brand entry,
including both halves of two renames in flight.

## What this ledger corrected in our own tooling

Twenty of its records were rejected by `scripts/check-ledger.mjs` on a rule
that was simply wrong, and finding that is worth more than the batch.

- **The panel-sum check was unsound.** It added protein + fat + moisture + ash
  + fibre and called anything over 100% impossible. Minima and maxima are
  BOUNDS: a perfectly ordinary Weruva can guarantees 12% protein min, 6% fat
  min and 83% moisture max, sums to 103.5%, and holds a food that is 13/7/79.
  Only the minima bound the food from below, so the one arithmetic
  impossibility is protein min plus fat min exceeding the whole pack. Twenty
  real panels failing is a check being loudest exactly where it is wrong.
- **The dry protein ceiling was a flat 50% as fed**, which is what kibble does
  and not what "dry" means. Weruva's freeze-dried raw is 60–66% protein at 8%
  moisture. It is now a share of dry matter — 75% for food, 95% for a treat.
- **The consumer sweep's data-suspect ceiling moved from 85% to 95%.** Weruva
  occupies the entire top of that distribution with ordinary meat-first cans;
  85.7% is 12% protein against 86% moisture, and every panel behind those
  numbers is real.
