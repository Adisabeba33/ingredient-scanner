...
## Seeded into the catalog — batch 022 (2026-08-29)

**All 60 `source_verified` records reached `data/known-products.ts` and
`data/known-formulas.ts`.** Nothing was held back: every record passed the
pre-flight checker (60 of 60 clean, including the calorie arithmetic on all
46 ounce-stated cans), no barcode collided with the 721 already seeded, with
`data/wrong-barcodes.ts`, or with any other research ledger.

They became **58 products**, because two flavours are sold in two can sizes
each and merge as pack sizes of one recipe rather than as separate products.

Registered at the same time, because Merrick was the first maker to need
them: GS1 prefix `022808` (Merrick keeps its own under Nestlé Purina), and
six range names the brand entry did not have — Chunky Grain Free,
Slow-Cooked BBQ, Kitchen Comforts, Backcountry Grain Free, and the two
Limited Ingredient Diet families. Without those the coverage page would have
filed a third of this batch under "Other".

The four `conflicts` are recorded verbatim as section H of
`docs/CATALOG-CONFLICTS.md`.

### Where the research stopped

This handoff's own conclusion stands and is the reason no further Merrick
work is queued: batch 3 consumed the clean single-size wet candidates, and
the remaining leads are named below under "Deliberately not added yet" and
"Best next research order" — Petite Pâté Turkey `022808010456` (unit vs
3-pack unproven), the cat Whitefish and Lamb 12 lb bag UPCs, and the dog dry
catalog, which is untouched. Anyone resuming should start there rather than
re-sweeping what is already here.
