...
Do not bulk-promote the old index UPC strings. A current first-party Shopify variant probe has already shown generation replacements in this queue. Example: current Tic Tac Whoa! variants expose `813778018418` (3 oz) and `813778018272` (5.5 oz), while the old index candidate for 5.5 oz is `813778018203`. Old identities must remain historical/conflict evidence unless the old individual unit itself is deliberately represented as a historical formula generation.
