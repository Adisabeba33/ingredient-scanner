...
| 6 | **Cesar** | dog | Mars | **SEEDED** — batch 042 | 54 barcodes, 29 compositions, all from cesar.com: barcode from its JSON-LD, composition from its label-panel images read twice. One campaign, not two — see `research/BRIEF-CESAR.md` §2, the template for the Mars brands below. 29 units wait on route B or a photograph. |
