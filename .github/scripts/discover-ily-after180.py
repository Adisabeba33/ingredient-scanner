...
ROOT=Path('.'); LED=ROOT/'research/deep-research-i-and-love-and-you.json'
led=json.loads(LED.read_text()); seen={r['upc'] for r in led['records']}
ex=''
...
for p in products:
    ...
    for v in d.get('variants',[]):
        ...
        if v.get('available') and b and b not in seen and b not in ex and ok(b):
            rows.append({'handle':h,'product':p['title'],'variant':v.get('title'),'barcode':b,'sku':v.get('sku')})
for row in rows: print('CAND',json.dumps(row,ensure_ascii=False))
print('UNIQUE',len({r['barcode'] for r in rows}))
